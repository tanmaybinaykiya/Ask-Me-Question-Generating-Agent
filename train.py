...
def main():
    ...
    decoder = DecoderLSTM(vocab_size=train_vocab_size, embedding_dim=300, hidden_dim=500, n_layers=1,
                          encoder_hidden_dim=500, embeddings=word_embeddings_glove)
    ...
    def greedy_search():
        ...
        for eachBatch in range(batch_per_epoch):
            ...
            while seq_len < max_len:
                ...
                eval_mode = True
            given_sentence = [
                [idx_to_word[str(answers[i][j].item())] for j in range(len(answers[i])) if answers[i][j] != 0]
                for i in range(len(answers))]
            ground_truth = [
                [idx_to_word[str(questions[i][j].item())] for j in range(len(questions[i])) if questions[i][j] != 0] for
                i
